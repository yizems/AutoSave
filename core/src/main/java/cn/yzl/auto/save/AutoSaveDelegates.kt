...
abstract class AbsAutoSave<T> {
    ...
    protected open fun readValueFromBundle(
        ...
        val bundle = AutoSaveHelper.getProvider(owner)
    ...
    fun register2Provider(owner: Any?, propertyName: String) {
        ...
        AutoSaveHelper.registerWithLifecycle(owner, owner.savedStateRegistry, owner)
        val provider = AutoSaveHelper.getProvider(owner) ?: return
 ...
 */
@Suppress("UNCHECKED_CAST")
class AutoSaveNullable<T> internal constructor(
    val init: (() -> T)? = null
) : AbsAutoSave<T>() {
 ...
 */
@Suppress("UNCHECKED_CAST")
class AutoSaveNotNull<T> internal constructor(
    val init: (() -> T)
) : AbsAutoSave<T>() {
 ...
 * @param T
 */
class AutoSaveLateInit<T> internal constructor() : AbsAutoSave<T>() {
 ...
 * 代理集合便捷方法
 */
object AutoSaveDelegates {
    /**
     * 可以为null 的代理
     * @return SavedDelegateNullable<T>
     */
    fun <T> nullable(init: (() -> T)? = null) = AutoSaveNullable<T>(init)
     ...
     */
    fun <T> notNull(init: (() -> T)) = AutoSaveNotNull(init)

    /**
     * lateinit var 实现,使用前需要初始化
     * @return SavedDelegateLateInit<T>
     */
    fun <T> lateInit() = AutoSaveLateInit<T>()
}
